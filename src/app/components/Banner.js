"use client"

import Image from "next/image";

import {MouseParallaxContainer, MouseParallaxChild} from "react-parallax-mouse";

const Banner = () => {
    return <section className="bg-primary bg-pattern lg:min-h-[768px] lg:pt-32  pt-16">
        <div className="container mx-auto min-h-[768px] flex items-center justify-center">
            <MouseParallaxContainer
                globalFactorX={0.4}
                globalFactorY={0.4}
                resetOnLeave
                className="w-full flex flex-col lg:flex-row items-center justify-between">
                <MouseParallaxChild factorX={0.1} factorY={0.2}>
                    <div
                        className="flex flex-col lg:flex-row items-center text-center lg:text-left flex-1 px-6 text-white">
                        <div className="text-[32px] font-bangers text-white">Best Pizza in town</div>
                        <h1 className="text-6xl lg:text-8xl font-bangers  drop-shadow-md">
                            Pizza perfection <br/> in every bite
                        </h1>
                    </div>
                </MouseParallaxChild>
                <MouseParallaxChild factorX={0.2} factorY={0.1} className="relative">
                    <div className="flex flex-col lg:flex-row items-center text-center lg:text-left flex-1 px-6">

                        <div className="flex-1 flex justify-end max-w-sm lg:max-w-max">
                            <Image
                                src="/pizza-banner.png"
                                alt="pizza"
                                width={550}
                                height={500}
                                priority={1}
                            />
                        </div>
                    </div>
                        <MouseParallaxChild factorX={0.2} factorY={0.3}
                                            className="absolute top-6 left-4 hidden xl:flex">
                            <Image src={"/chilli-1.png"} width={160} height={84} alt=""/>
                        </MouseParallaxChild>
                        <MouseParallaxChild factorX={0.2} factorY={0.3}
                                            className="absolute top-16 left-4 hidden xl:flex">
                            <Image src={"/chilli-2.png"} width={130} height={84} alt=""/>
                        </MouseParallaxChild>
                        {/*garlic 1*/}
                        <MouseParallaxChild factorX={0.2} factorY={0.3}
                                            className="absolute top-80 -left-24 hidden xl:flex">
                            <Image src={"/garlic-1.png"} width={84} height={72} alt=""/>
                        </MouseParallaxChild>
                        {/*garlic 2*/}
                        <MouseParallaxChild factorX={0.3} factorY={0.6}
                                            className="absolute top-80 -left-12 hidden xl:flex">
                            <Image src={"/garlic-2.png"} width={100} height={72} alt=""/>
                        </MouseParallaxChild>

                        {/*garlic 3*/}
                        <MouseParallaxChild factorX={0.3} factorY={0.6}
                                            className="absolute top-96 -left-20 hidden xl:flex">
                            <Image src={"/garlic-3.png"} width={100} height={72} alt=""/>
                        </MouseParallaxChild>
                        {/*    leaves*/}
                        <MouseParallaxChild factorX={0.3} factorY={0.6}
                                            className="absolute top-96 left-20 hidden xl:flex">
                            <Image src={"/leaves.png"} width={180} height={72} alt=""/>
                        </MouseParallaxChild>

                </MouseParallaxChild>
            </MouseParallaxContainer>
        </div>
    </section>;
};

export default Banner;
